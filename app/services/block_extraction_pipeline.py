...
class BlockExtractionPipeline:
    ...
    def _detect_blocks(self, content: str) -> dict[str, str]:
        ...
        narrative_start_re = re.compile(r"^I\.\s*", re.IGNORECASE)
        table_anchor_re = re.compile(r"BIEU\s*MAU\s*THONG\s*KE", re.IGNORECASE)
        ...
        if narrative_start is None:
            narrative_start = min(30, len(lines))
    ...
    def _extract_header(self, header_text: str) -> BlockHeader:
        ...
        first_30 = "\n".join([line for line in header_text.splitlines() if line.strip()][:30])

        # Deterministic date extraction for header to avoid picking report period dates.
        ...
        extracted = self._extract_block(first_30, BlockHeader, "Header")
    ...
    @staticmethod
    def _parse_phan_nghiep_vu_fallback(text: str) -> BlockNghiepVu:
        ...
        tong_so_vu_chay = extract_after_keyword([
            r"TONG\s*SO\s*VU\s*CHAY\D*(\d+)",
            r"SO\s*VU\s*CHAY\D*(\d+)",
        ])
        tong_so_vu_no = extract_after_keyword([
            r"TONG\s*SO\s*VU\s*NO\D*(\d+)",
            r"SO\s*VU\s*NO\D*(\d+)",
        ])
        tong_so_vu_cnch = extract_after_keyword([
            r"TONG\s*SO\s*VU\s*TAI\s*NAN\s*SU\s*CO\D*(\d+)",
            r"TONG\s*SO\s*VU\s*CNCH\D*(\d+)",
            r"SO\s*VU\s*CNCH\D*(\d+)",
        ])
        quan_so_truc = extract_after_keyword([
            r"QUAN\s*SO\s*TRUC\D*(\d+)",
            r"SO\s*QUAN\s*SO\s*TRUC\D*(\d+)",
        ])

        chi_tiet_lines: list[str] = []
        for line in lines:
            nline = BlockExtractionPipeline._normalize(line)
            if "CNCH" in nline or "TAI NAN" in nline or "SU CO" in nline:
                chi_tiet_lines.append(line)
        chi_tiet_cnch = "\n".join(chi_tiet_lines[:12]).strip()
    ...
    @staticmethod
    def _parse_bang_thong_ke_from_text_grid(table_text: str) -> BlockBangThongKe:
        ...
        for stt, noi_dung, value in rows:
            ...
            if "DANH MUC" in upper_noi_dung or "CHI TIEU" in upper_noi_dung:
                continue
    ...
    @staticmethod
    def _parse_bang_thong_ke_from_tables(table_stream: list[list[list[str]]]) -> BlockBangThongKe:
        seen: set[str] = set()
        items: list[ChiTieu] = []
        for table in table_stream or []:
            for row in table:
                ...
                normalized = BlockExtractionPipeline._normalize(joined)
                if "DANH MUC" in normalized or "CHI TIEU" in normalized:
                    continue
                ...
                number_matches = re.findall(r"-?\d+", joined)
                ket_qua = int(number_matches[-1]) if len(number_matches) >= 2 else 0

                noi_dung = joined
                noi_dung = re.sub(rf"\b{re.escape(stt)}\b", "", noi_dung, count=1)
                if len(number_matches) >= 2:
                    noi_dung = re.sub(rf"\b{re.escape(str(ket_qua))}\b\s*$", "", noi_dung)
    ...
    def _enforce_schema(self, header: BlockHeader, header_context: str) -> BlockHeader:
        ...
        required_fields = ["so_bao_cao", "ngay_bao_cao", "thoi_gian_tu_den", "don_vi_bao_cao"]
        ...
        for field_name in required_fields:
            ...
            messages = [
                {
                    "role": "system",
                    "content": f"Find ONLY {field_name}. Return JSON with one key: value.",
                },
                {"role": "user", "content": header_context[:3000]},
            ]
            try:
